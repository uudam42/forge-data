...
class LocalCleanedArtifactStore(CleanedArtifactStore):
    ...
    def staging_dir(self, *, synchronization_id: str, cleaning_id: str) -> Path:
        ...
        # exist_ok=False (enforced inside create_staging_dir): cleaning_id
        # is UUID4, so a collision should never happen — fail safe rather
        # than write into a stale directory.
        create_staging_dir(
            staging,
            operation_id=cleaning_id,
            artifact_id=cleaning_id,
            stage="cleaning",
    ...
    def commit(self, *, synchronization_id: str, cleaning_id: str, staging_dir: Path) -> str:
        ...
        try:
            ...
            raise CleanedArtifactAlreadyExistsError(f"Cleaning run already exists: {final_dir}") from exc
